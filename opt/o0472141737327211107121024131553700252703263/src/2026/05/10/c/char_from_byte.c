...
/*
    ...
    "[ an byte type ], if the value can be
    "represented by the [ byte ] type, it
    "is unchanged.
    "  Otherwise... the value is converted by
    "repeatedly adding... one more than [ UCHAR_MAX ]
    "until the value is in the range of [ byte ]."

    In converting from { byte } to { char },
    we assume that the { byte } value was
...
STATIC_ASSERT ( CHAR_MIN <= ( 1 + CHAR_MAX ) - ( UCHAR_MAX + 1 ) ) ;

const char char_from_byte (
        const byte u )
{
